...
const TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      categoria_gasto_op: { type: ["string", "null"], enum: ["sueldo", "arriendo", "marketing", "software", "contabilidad", "otro", null], description: "Categoría de gasto operacional: sueldo, arriendo, marketing, software, contabilidad, otro" },
      cliente_nombre: { type: ["string", "null"], description: "Nombre del cliente que pagó/cobró, para ingresos" },
      periodo_mes: { type: ["string", "null"], description: "Mes del período en formato YYYY-MM si menciona 'junio', 'este mes', mes específico. Inferir del contexto: 'junio 2026' → '2026-06', 'este mes' → mes actual" },
      nombre_persona: { type: ["string", "null"], description: "Nombre de persona mencionada, ej: 'sueldo de Rodrigo' → 'Rodrigo'" },
    },
    required: ["intent"],
  },
};
...
function systemPrompt({ rol, catalogos, estadoPendiente, hayImagen }) {
  return [
    ...
    hayImagen
      ...
      : "",
    "REGLAS CRÍTICAS v3:",
    "• REGLA 1 — Imagen = SIEMPRE REGISTRAR_RENDICION salvo pago explícito: Si hay imagen Y el texto NO dice 'marcar pagado'/'pagar rendición'/'pago la [número]' con ID numérico → REGISTRAR_RENDICION. La palabra 'pago/pagos' en descripción NO es pago de rendición.",
    "• REGLA 2 — monto_imagen: Extraer SOLO campo etiquetado 'Monto'/'Monto pagado'/'Total'/'Subtotal'. NUNCA mezclar con factura/pedido/RUT/referencia/cuenta.",
    "• REGLA 3 — Mensajes reenviados: Si texto empieza con 'Reenviado' o es obvio que es reenviado → REGISTRAR_RENDICION con imagen, DESCONOCIDO sin imagen.",
    "• REGISTRAR_GASTO_OPERACIONAL: 'sueldo/arriendo/marketing/software/contabilidad' SIN obra → gasto operacional. Extrae categoria_gasto_op, monto, periodo_mes, nombre_persona si menciona (ej: 'sueldo Rodrigo').",
    "• REGISTRAR_INGRESO: 'cobré/recibí pago/me pagaron/abono cliente' → ingreso. Área inferida: obra→Construcción, diseño/planos→Arquitectura.",
    "• CONSULTAR_ESTADO_RESULTADOS: SOLO cuando Gary pide explícitamente 'estado de resultados'/'ER'/'resultado del mes'/'cómo vamos financieramente'. NO proactivo.",
