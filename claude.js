...
function systemPrompt({ rol, catalogos, estadoPendiente, hayImagen }) {
  return [
    ...
    hayImagen && !estadoPendiente
      ? "El mensaje trae una imagen adjunta (boleta/factura/comprobante). NUNCA uses DESCONOCIDO solo porque no hay texto — clasifica SIEMPRE como REGISTRAR_RENDICION (un gasto nuevo) e intenta leer monto, proveedor, fecha y tipo de documento de la imagen. Usa REGISTRAR_PAGO únicamente si el texto pide explícitamente marcar como pagada una rendición YA EXISTENTE (ej: 'marcar pagado', 'pagar rendición 42', 'pago la 42', o menciona un ID de rendición) — que el texto contenga la palabra 'pago'/'pagos' como parte de la descripción de una compra (ej: 'pagos cemento etapa 1') NO califica para REGISTRAR_PAGO, sigue siendo una rendición nueva. Si la imagen está borrosa o no se puede leer el monto, de todas formas usa REGISTRAR_RENDICION con legible_imagen=false y monto_imagen=null — el sistema le preguntará el monto al usuario."
      : "",
    hayImagen
