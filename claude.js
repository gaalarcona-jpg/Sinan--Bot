...
const TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      cliente_nombre: { type: ["string", "null"], description: "Nombre SOLO del cliente que pagó, para ingresos. Extraer únicamente el nombre personal o empresa, descartando descriptores geográficos (comuna, ciudad, sector, ubicación). Ejemplos: 'cliente Walter Comuna de Quilpue' → 'Walter'; 'Gonzalo Parrague Padre Hurtado' → 'Gonzalo Parrague'; 'Juan Martínez Santiago' → 'Juan Martínez'. Nunca incluir el nombre de la ciudad, comuna ni dirección como parte del nombre." },
      comprobante_pdf: { type: ["boolean", "null"], description: "true si el usuario envió un PDF como comprobante de un pago recibido (ingreso). false o null en cualquier otro caso." },
      tipo_inversion: { type: ["string", "null"], enum: ["activo_fijo", "herramienta", "vehiculo", "material_stock", "otro", null], description: "Tipo de inversión para REGISTRAR_INVERSION: activo_fijo (equipos/maquinaria/computadores), herramienta (herramientas manuales/eléctricas), vehiculo, material_stock (materiales almacenados sin asignar a obra), otro" },
      descripcion_activo: { type: ["string", "null"], description: "Descripción del activo adquirido en una inversión (ej: 'andamio 6 metros', 'notebook Dell', 'camioneta Ford Ranger 2023', 'taladro inalámbrico Makita')" },
      vida_util_anos: { type: ["number", "null"], description: "Vida útil estimada en años del activo de inversión, si el usuario la menciona" },
...
function systemPrompt({ rol, catalogos, estadoPendiente, hayImagen }) {
  return [
    ...
    "• REGISTRAR_INVERSION: cuando Gary menciona compra de activo fijo, herramienta, vehículo, material en stock o equipamiento sin asignar a una obra específica (ej: 'compré un andamio', 'compramos una camioneta', 'adquirí notebook para la oficina', 'compré herramientas'). Extrae tipo_inversion, descripcion_activo, monto, proveedor, vida_util_anos si se menciona. IMPORTANTE: si la compra es para una obra/etapa específica, usa REGISTRAR_RENDICION en vez de REGISTRAR_INVERSION.",
